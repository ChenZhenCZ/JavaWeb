...
import com.gs.activiti.User;
import com.gs.bean.BossTask;
...
public class ActivitiController {
    ...
    public void viewProc(HttpServletResponse resp) throws IOException {
        ...
        List<Model> models = repositoryService.createModelQuery().deploymentId("17501").list();
        for (Model model : models) {
            model.getId();
        }
        List<String> names = repositoryService.getDeploymentResourceNames("17501");
        ...
        InputStream in = repositoryService.getResourceAsStream("17501", imageName);
        /**
    ...
    public void viewProc1(HttpServletResponse resp) throws IOException {
        ...
        InputStream in = new DefaultProcessDiagramGenerator().generateDiagram(
                repositoryService.getBpmnModel("leave_process_complex:1:17504"), "png", activityIds);
        OutputStream out = resp.getOutputStream();
        byte[] b = new byte[1024];
        for (int len = -1; (len= in.read(b)) != -1;) {
            out.write(b, 0, len);
