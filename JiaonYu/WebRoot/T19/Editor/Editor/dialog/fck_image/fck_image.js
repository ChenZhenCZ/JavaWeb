...
window.onload = function()
{
	...
	//缩略图赋值	
	GetE('IsWatermark').value	= FCKConfig.IsWatermark;
	GetE('IsThumb').value		= FCKConfig.IsThumb;		
    //字段赋值
...
function Ok()
{
    ...
    if (FCKConfig.ImgPreview == true) {
        oEditor.imgUrl(GetE('txtUrl').value,FCKConfig.IsThumb);
    }
...
function UpdateImage( e, skipId )
{
	...
	//SetAttribute( e, "onmousewheel" , "return bbimg(this)" ) ;
	//SetAttribute( e, "onload" ,"resizepic(this)" ) ;	
...
function OnUploadCompleted( errorNumber, fileUrl, fileName, customMsg )
{
	switch ( errorNumber )
	{
		...
		case 101 :	// Custom warning
			...
			break ;
		case 201 :
			alert( '已经存在具有相同名字的文件名，上传文件重命名为："' + fileName + '"' ) ;
			break ;
		case 202 :
			alert( '上传文件类型不对！请上传以"' + customMsg + '"为后缀名的文件！' ) ;
			return ;
		case 203 :
			alert( "安全性错误：你可能没有足够的上传权限，请检查你的服务器。" ) ;
			return ;
		case 204 :
...
function CheckUpload()
{
	...
	{
		OnUploadCompleted( 202,'','',FCKConfig.ImageUploadAllowedExtensions.replace(".", "").replace("$", "")) ;
		return false ;
	}
	return true ;
}