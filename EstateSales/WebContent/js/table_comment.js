...
$(function(){
	$(".detail").click(function(){
		...
		if(data.length==1){
			var time = formatDate(data[0].comment_time);
			$("#commentDetail").fill(data[0]);
...
$(function(){
	$(".del").click(function(){
		var data=$("#tb").bootstrapTable("getSelections");
		if(data.length==1){
			$.post("/EstateSales/comment/delete", 
					{"comment.id":data[0].id},
					function(msg) {
						$("#tb").bootstrapTable("refresh",{url:"/EstateSales/comment/listPage"});
						toastr.success("成功删除!"); 
					}, "json"
				);
		}else{
